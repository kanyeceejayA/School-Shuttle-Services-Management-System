jQuery(document).ready(function($) {
    ...
    function getAllRoutesForMap() {
        ...
        showPermanentMessage('Please select a shuttle Route below');
    ...
    function loadRoutes(json) {        
        if (json.length == 0) {
            ...
            option.innerHTML = 'Select shuttle Route...';
            ...
            $(json.routes).each(function(key, value){
                ...
                option.innerHTML = $(this).attr('shuttleno') + " " + $(this).attr('times');
                routeSelect.appendChild(option);
            });
    ...
    function createMarker(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
                          shuttleno, sessionID, accuracy, extraInfo, map, finalLocation) {
        var iconUrl;
        var iconSizes;

        if (finalLocation) {
            iconUrl = 'images/new/shuttle_bus_stop.png';
            iconSizes = [40, 36];
        } else {
        ...
        if (finalLocation) {
            lastMarker = "</td></tr><tr><td align=left>&nbsp;</td><td><b>Current location</b></td></tr>";
        }

        // convert from meters to feet if need be
        ...
        var popupWindowText = "<table border=0 style=\"font-size:95%;font-family:arial,helvetica,sans-serif;color:#000;\">" +
            ...
            "<tr><td align=right>NumberPlate:&nbsp;</td><td>" + shuttleno + "</td><td>&nbsp;</td></tr>" +
            "<tr><td align=right>Accuracy:&nbsp;</td><td>" + accuracy + " meters</td><td>&nbsp;</td></tr></table>";


        var gpstrackerMarker;
        var title = shuttleno + " - " + gpsTime;
